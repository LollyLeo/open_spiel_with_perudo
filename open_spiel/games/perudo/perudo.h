...
#ifndef OPEN_SPIEL_GAMES_PERUDO_H_
#define OPEN_SPIEL_GAMES_PERUDO_H_
...
//   // they believe prevalent if another player increased the face value on
//   their
//   // bid).
//   kResetFace = 1,
...
class PerudoState : public State {
public:
  explicit PerudoState(std::shared_ptr<const Game> game, int total_num_dice,
                       int max_dice_per_player,
                       const std::vector<int> &num_dice);
  PerudoState(const PerudoState &) = default;
...
private:
  ...
  int loser_;
  int current_bid_;
  int64_t min_allowed_ones_;
  int64_t min_allowed_simple_;
  int total_num_dice_;
  int total_moves_;
...
class PerudoGame : public Game {
public:
  explicit PerudoGame(const GameParameters &params, GameType game_type);
...
class ImperfectRecallPerudoState : public PerudoState {
public:
  ImperfectRecallPerudoState(std::shared_ptr<const Game> game,
                             int total_num_dice, int max_dice_per_player,
                             const std::vector<int> &num_dice)
      : PerudoState(game, total_num_dice, max_dice_per_player, num_dice) {}
  std::string InformationStateString(Player player) const override;
  std::unique_ptr<State> Clone() const override {
    return std::unique_ptr<State>(new ImperfectRecallPerudoState(*this));
  }
};

class ImperfectRecallPerudoGame : public PerudoGame {
public:
  explicit ImperfectRecallPerudoGame(const GameParameters &params);
  std::unique_ptr<State> NewInitialState() const override;
...
#endif // OPEN_SPIEL_GAMES_PERUDO_H_
