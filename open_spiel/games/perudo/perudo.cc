...
#include "open_spiel/games/perudo/perudo.h"
...
const GameType kGameType{/*short_name=*/"perudo",
                         /*long_name=*/"Perudo",
...
const GameType kImperfectRecallGameType{
    /*short_name=*/"perudo_ir",
    /*long_name=*/"Perudo with Imperfect Recall",
    GameType::Dynamics::kSequential,
    GameType::ChanceMode::kExplicitStochastic,
    GameType::Information::kImperfectInformation,
    GameType::Utility::kZeroSum,
...
std::shared_ptr<const Game> Factory(const GameParameters &params) {
  return std::shared_ptr<const Game>(new PerudoGame(params, kGameType));
}

std::shared_ptr<const Game>
ImperfectRecallFactory(const GameParameters &params) {
  return std::shared_ptr<const Game>(new ImperfectRecallPerudoGame(params));
}

const PerudoGame *UnwrapGame(const Game *game) {
  return down_cast<const PerudoGame *>(game);
}
} // namespace
...
PerudoState::PerudoState(std::shared_ptr<const Game> game, int total_num_dice,
                         int max_dice_per_player,
                         const std::vector<int> &num_dice)
    : State(game), dice_outcomes_(), bidseq_(),
      cur_player_(kChancePlayerId), // chance starts
      cur_roller_(0),               // first player starts rolling
      winner_(kInvalidPlayer), loser_(kInvalidPlayer),
      current_bid_(kInvalidBid), min_allowed_ones_(0), min_allowed_simple_(1),
      total_num_dice_(total_num_dice), total_moves_(0), calling_player_(0),
      bidding_player_(0), max_dice_per_player_(max_dice_per_player),
      num_dice_(num_dice), num_dice_rolled_(game->NumPlayers(), 0),
      bidseq_str_() {
...
std::string PerudoState::ActionToString(Player player, Action action_id) const {
...
int PerudoState::CurrentPlayer() const {
  if (IsTerminal()) {
...
// changed !
void PerudoState::ResolveWinner() {
  ...
  for (auto p = Player{0}; p < num_players_; p++) {
    for (int d = 0; d < num_dice_[p]; d++) {
      if (dice_outcomes_[p][d] == face || dice_outcomes_[p][d] == 1) {
        matches++;
      }
    }
  }
...
const int PerudoState::dice_sides() const {
  return UnwrapGame(game_.get())->dice_sides();
}

// changed !
void PerudoState::DoApplyAction(Action action) {
  ...
  if (IsChanceNode()) {
    ...
    bool is_simple;
    if (bidseq_.empty()) {
      correct_action = (action != total_num_dice_ * dice_sides());
      is_simple = (action % dice_sides() != 0);
    } else {
      if (action % dice_sides() == 0) {
        correct_action = (action >= min_allowed_ones_);
        is_simple = false;
      } else {
        correct_action = (action >= min_allowed_simple_);
        is_simple = true;
      }
    }
    if (!correct_action) {
    ...
    if (action == total_num_dice_ * dice_sides()) {
      ...
      // update allowed
      int64_t count = action / dice_sides() + 1;
      if (is_simple) {
        min_allowed_simple_ = action + 1;
        if (min_allowed_simple_ % dice_sides() == 0) {
          ++min_allowed_simple_;
        }
        min_allowed_ones_ =
            std::max(min_allowed_ones_, ((count - 1) / 2) * dice_sides());
      } else {
        min_allowed_ones_ = action + dice_sides();
        min_allowed_simple_ =
            std::max(min_allowed_simple_, (count * 2 - 1) * dice_sides() + 1);
      }
    }
    total_moves_++;
  }
...
// changed !
// may be shity chatGPT code
std::vector<Action> PerudoState::LegalActions() const {
  if (IsTerminal())
    return {};
  if (IsChanceNode())
    return LegalChanceOutcomes();

  const int64_t S = dice_sides();
  const int64_t max = total_num_dice_ * S;

  std::vector<Action> actions;
  actions.reserve(max); // tight upper bound

  int64_t k = min_allowed_ones_;
  int64_t x = min_allowed_simple_;

  // Align x to the first non-multiple of S
  int64_t block = x / S;
  int64_t offset = x % S;
  if (offset == 0)
    offset = 1;

  while (k < max || block * S + offset < max) {
    int64_t xv = block * S + offset;

    if (k < max && (xv >= max || k < xv)) {
      actions.push_back(k);
      k += S;
    } else {
      actions.push_back(xv);
  ...
  if (!bidseq_.empty()) {
    actions.push_back(max);
  }

  return actions;
}

std::vector<std::pair<Action, double>> PerudoState::ChanceOutcomes() const {
...
std::string PerudoState::InformationStateString(Player player) const {
...
std::string PerudoState::ToString() const {
  std::string result = "";
...
bool PerudoState::IsTerminal() const { return winner_ != kInvalidPlayer; }

std::vector<double> PerudoState::Returns() const {
...
void PerudoState::InformationStateTensor(Player player,
...
void PerudoState::ObservationTensor(Player player,
                                    absl::Span<float> values) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
...
std::unique_ptr<State> PerudoState::Clone() const {
  return std::unique_ptr<State>(new PerudoState(*this));
}

// no change
std::pair<int, int> PerudoState::UnrankBid(int bidnum) const {
...
PerudoGame::PerudoGame(const GameParameters &params, GameType game_type)
...
int PerudoGame::NumDistinctActions() const {
  return total_num_dice_ * dice_sides_ + 1;
}

std::unique_ptr<State> PerudoGame::NewInitialState() const {
  std::unique_ptr<PerudoState> state(
      new PerudoState(shared_from_this(),
...
int PerudoGame::MaxChanceOutcomes() const { return dice_sides_; }

int PerudoGame::MaxGameLength() const {
  // A bet for each side and number of total dice, plus "liar" action.
  return total_num_dice_ * dice_sides_ + 1;
}
int PerudoGame::MaxChanceNodesInHistory() const { return total_num_dice_; }

std::vector<int> PerudoGame::InformationStateTensorShape() const {
...
std::vector<int> PerudoGame::ObservationTensorShape() const {
  // One-hot encoding for the player number.
  // One-hot encoding for each die (max_dice_per_player_ * sides).
...
ImperfectRecallPerudoGame::ImperfectRecallPerudoGame(
    const GameParameters &params)
    : PerudoGame(params, kImperfectRecallGameType),
      recall_length_(
          ParameterValue<int>("rollout_length", kDefaultRecallLength)) {}

std::unique_ptr<State> ImperfectRecallPerudoGame::NewInitialState() const {
  return absl::make_unique<ImperfectRecallPerudoState>(
      shared_from_this(),
...
std::string
ImperfectRecallPerudoState::InformationStateString(Player player) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  const auto *parent_game =
      down_cast<const ImperfectRecallPerudoGame *>(game_.get());
  ...
  int start_index =
      std::max<int>(bidseq_.size() - parent_game->recall_length(), 0);
